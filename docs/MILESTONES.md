...
### Tasks
- [ ] Create `apps/drone/` with `Dockerfile` extending `ghcr.io/nap-it/vanetza-nap:latest` (bundled single image)
...
- [ ] Implement `vanetza_client.py`: thin wrapper to publish/subscribe CAM and DENM on the local vanetza broker (127.0.0.1:1883)
...
- [ ] Add `drone` service to `docker-compose.dev.yml` with `deploy.replicas`, Docker socket mount, base station position env vars
...
- [ ] Wire coverage into `DroneAgent`:
  ...
  - On receiving CAM with `stationType=10`: transition to `COLLECTING`
    - Look up sensor IP from `sim/announce` registry, connect to `{sensor_ip}:1883`, publish `sensor/request_data`, await `sensor/data_response`
